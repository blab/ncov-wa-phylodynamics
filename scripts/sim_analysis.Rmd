...
for (rep in seq(0,8)){
  ...
  intro_times$date = as.Date("2020-02-06") + floor(intro_times$t*366)
...
g.perc_intros = ggplot(growth) +
  geom_line(data=smooth_intro[which(smooth_intro$timeframe!=2),], aes(x=t,y=y*100, color="simulated daily cases\nfrom introdcutions"))+
...
plot(g.perc_intros)
ggsave(plot=g.perc_intros, file=paste(path, 'figures/sim_intro_percentage.png', sep='/'), height=6,width=7)
