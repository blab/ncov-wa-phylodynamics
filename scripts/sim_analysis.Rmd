...
for (i in seq(1,length(uni_dates))){
  new.positive = data.frame(date=uni_dates[[i]], count=sum(intro_times$date==uni_dates[[i]]), detection="Positive")
  if (i==1){
...
```{r smapling}
mrsi = data.frame(filename="simmulticoal_skygrid_2",date=as.Date("2020-03-24"))
mrsi = rbind(mrsi, data.frame(filename="simmultibd_skygrid_2",date=as.Date("2020-03-24")))
