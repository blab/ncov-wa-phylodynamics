...
timeframename = c('WA w/o Yakima', 'D clade', 'G clade', 'Yakima')
...
dat$timeframe <- factor(dat$timeframe, levels = c("D clade", "G clade", "WA w/o Yakima", "Yakima"))
...
p = ggplot(dat_tmp[which(dat_tmp$method=="skygrowth" & dat_tmp$timeframe!="Yakima"),]) + 
  ...
  geom_histogram(data=testing, aes(x=date,y=count), stat="identity")+
  ...
  scale_fill_manual(name="Clade:", values = c("WA w/o Yakima"=both_clades_col, "D clade"=d_clade_col, "G clade"=g_clade_col))+
  theme_minimal() +
...
p = ggplot(dat) + 
  ...
  scale_fill_manual(name="Dataset:", values = c("WA w/o Yakima"=both_clades_col, "D clade"=d_clade_col, "G clade"=g_clade_col, "Yakima"=yakima_col))+
...
p = ggplot(ltt.dat) + geom_ribbon(aes(x=time, ymin=val_offset-val/2, ymax=val_offset+val/2, group=cluster, fill=clade)) +
  ...
  geom_text(aes(x=as.Date("2020-02-4"), label="0", y=250), colour="black", text=element_text(size=3))+
  geom_text(aes(x=as.Date("2020-02-4"), label="50", y=300), colour="black", text=element_text(size=3))+
  geom_text(aes(x=as.Date("2020-02-4"), label="100", y=350), colour="black", text=element_text(size=3))+
...
subset = mobility[which(mobility$region=="North" | mobility$region=="King" | mobility$region=="Pierce" | mobility$sub_region_2=="Yakima County"),]
...
for (l in seq(10,length(mob.val)-2)){
  subset$val = subset[,mob.val[[l]]]
  
  subset[which(as.numeric(subset$date-sat) %% 7==0), "val"] = NA
  subset[which(as.numeric(subset$date-sun) %% 7==0), "val"] = NA
...
p = ggplot(data=dat.bdsky_tmp[which(dat.bdsky_tmp$timeframe!="Yakima"),]) + 
  geom_line(data=subset[which(subset$sub_region_2!="Yakima County"),], aes(x=date, y=val/30+2.5,color=region, group=sub_region_2))+
  ...
  scale_color_manual(name="Mobility in County:", values=c("#000000", "#8B0000", "#FF6347"))+ 
  theme_minimal() +
...
ggsave(plot=p + ylab("")+scale_y_continuous(sec.axis = sec_axis(~ ., name="")), file=paste(path, 'figures/bdsky_R0_mobility_withlegend.pdf', sep='/'), height=1.5,width=4.5)
...
p = ggplot(data=growth_tmp[which(growth_tmp$method=="skygrowth" & growth_tmp$timeframe!="Yakima"), ]) + 
  ...
  scale_fill_manual(name="Dataset:", values = c("WA w/o Yakima"=both_clades_col, "D clade"=d_clade_col, "G clade"=g_clade_col, "Yakima"=yakima_col))+
  scale_y_continuous(sec.axis = sec_axis(~ .*30 - 75, name="workplace mobility")) +
  scale_color_manual(name="Counties:", values=c("#000000", "#8B0000", "#FF6347"))+ 
  theme_minimal() +
  # theme(legend.position = "none") + 
  xlab("") +
  ylab("effective reproduction number")+
...
ggsave(plot=p + theme(legend.position = "none"), file=paste(path, 'figures/coal_R0_mobility.png', sep='/'), height=2.5,width=6)


p = ggplot(data=dat.bdsky[which(dat.bdsky$timeframe=="Yakima"),]) + 
  geom_ribbon(aes(x=time, ymin=Ne.lower, ymax=Ne.upper, fill=timeframe), alpha=0.2) +
  geom_ribbon(aes(x=time, ymin=Ne.ll, ymax=Ne.uu, fill=timeframe), alpha=0.8) +
  geom_line(data=subset[which(subset$sub_region_2=="Yakima County" | subset$sub_region_2=="King County"  | subset$sub_region_2=="Pierce County"),], aes(x=date, y=val/30+2.5, group=sub_region_2, color=sub_region_2))+
  
  scale_x_date(limits=c(as.Date('2020-01-25'), max(mrsi$date)))  +
  scale_fill_manual(name="Dataset:", values = c("WA w/o Yakima"=both_clades_col, "D clade"=d_clade_col, "G clade"=g_clade_col, "Yakima"=yakima_col))+
  scale_color_manual(name="Mobility in County:", values=c("#000000", "#FF6347", yakima_col))+ 
  scale_y_continuous(sec.axis = sec_axis(~ .*30 - 75, name="workplace mobility")) +
  theme_minimal() +
  # theme(legend.position = "none") + 
  xlab("") +
...
ggsave(plot=p, file=paste(path, 'figures/bdsky_R0_mobility_yakima_legend.pdf', sep='/'), height=1.5,width=4)
