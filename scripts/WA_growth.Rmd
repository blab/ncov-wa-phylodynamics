...
p = ggplot(dat[which(dat$method=="correlated Ne's" & dat$timeframe=="samples until 2020-03-24"),]) + 
  ...
  geom_ribbon(aes(x=time, ymin=Ne.ll*transform, ymax=Ne.uu*transform, fill=timeframe), alpha=0.2)+
...
```{r sampling prob}
sampling = read.table(paste(path,'/results/mrsi.tsv',sep="/"), sep="\t", header=T)
...
p = ggplot(prev[which(prev$method=="correlated Ne's" & prev$timeframe==timeframename[[length(timeframename)]]),]) + geom_line(aes(x=time, y=I.mean, color=timeframe)) +
