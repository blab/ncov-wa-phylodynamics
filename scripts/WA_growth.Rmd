...
timeframename = c('UW Virology + 50 random samples', 'UW Virology + 150 random samples')
...
p = ggplot(dat[which(dat$method=="correlated Ne trajectories" & dat$timeframe==timeframename[[1]]),]) + 
  ...
  geom_histogram(data=testing, aes(x=date-reporting_delay,y=count), sta="identity")+
  ...
  coord_cartesian(ylim=c(0.01,2500)) +
  xlab("")
...
ggsave(plot=p, file=paste(path, 'figures/coal_Ne_vs_test.png', sep='/'), height=3,width=9)

p = ggplot(dat[which(dat$method=="correlated Ne trajectories" & dat$timeframe==timeframename[[2]]),]) + 
...
```{r comparison}
p = ggplot(dat) + geom_line(aes(x=time, y=log(Ne.mean), color=timeframe)) +
...
p_coal_growth = ggplot(growth[which(growth$method=="correlated Ne trajectories" & growth$timeframe==timeframename[[1]]),]) + 
  ...
  geom_ribbon(data=dat.bdsky[which(dat.bdsky$timeframe==timeframename[[1]]),], aes(x=time, ymin=growth.lower, ymax=growth.upper, fill="birth-death skyline"), alpha=0.2) +
  geom_ribbon(data=dat.bdsky[which(dat.bdsky$timeframe==timeframename[[1]]),], aes(x=time, ymin=growth.ll, ymax=growth.uu, fill="birth-death skyline"), alpha=0.8) +
...
p_R0 = ggplot(growth[which(growth$method=="correlated Ne trajectories" & growth$timeframe==timeframename[[1]]),]) + 
  ...
  geom_ribbon(data=dat.bdsky[which(dat.bdsky$timeframe==timeframename[[1]]),], aes(x=time, ymin=Ne.lower, ymax=Ne.upper, fill="birth-death skyline"), alpha=0.2)+
  geom_ribbon(data=dat.bdsky[which(dat.bdsky$timeframe==timeframename[[1]]),], aes(x=time, ymin=Ne.ll, ymax=Ne.uu, fill="birth-death skyline"), alpha=0.8)+
...
first = T
for (i in seq(0,length(cluster_member)-1,1)){
  nr_intros_samples = c()
  for (r in seq(1,1000)){
...
g.nrintros = ggplot(nr_intros, aes(x=nr_samples, y = nr_intros)) +
  geom_line()+
  # geom_ribbon(aes(x=nr_samples, ymin=lower, ymax=upper), alpha=0.2)+
  # geom_ribbon(aes(x=nr_samples, ymin=ll, ymax=uu), alpha=0.8)+
  theme_minimal()+
  scale_y_log10()+
...
g.perc_intros = ggplot(growth[which(growth$method=="correlated Ne trajectories" & growth$timeframe==timeframename[[1]]),]) +
  geom_ribbon(aes(x=time, ymin=intro.l, ymax=intro.u), alpha=0.2)+
  geom_ribbon(aes(x=time, ymin=intro.ll, ymax=intro.uu), alpha=0.8)+
  theme_minimal()+
  scale_y_log10()+
  scale_x_date()+
