...
timeframename = c('both clades', 'D clade', 'G clade')
...
onset_cases = read.table(paste(path,'/data/PUBLIC-CDC-Event-Date-SARS.csv',sep=""), sep=",", header=T)
  ...
  if (!startsWith( as.character(mrsi[i,"filename"]),"multibd")){
    ...
    for (j in seq(1,length(time))){
      ...
      new.dat = data.frame(time=date_val, 
                           ...
                           nr_cases = testing[which(testing$date==date_val), "count"],
...
p = ggplot(dat[which(dat$method=="correlated Ne trajectories"),]) + 
    geom_vline(xintercept=vline1, alpha=0.5)+
  geom_vline(xintercept=vline2, alpha=0.5)+
  geom_vline(xintercept=vline3, alpha=0.5)+
  geom_text(aes(x=vline1, label="local spread\nannounced", y=text_height), colour="black", angle=90, text=element_text(size=8)) +
    geom_text(aes(x=vline2, label="initial measures taken\nby state government", y=text_height), colour="black", angle=90, text=element_text(size=8)) +
      geom_text(aes(x=vline3, label="stay home stay healthy\nby state government", y=text_height), colour="black", angle=90, text=element_text(size=8)) +
  # geom_line(aes(x=time, y=Ne.mean*transform, color=timeframe)) +
  geom_histogram(data=testing, aes(x=date-5,y=count), sta="identity")+
  ...
  scale_fill_manual(name="Clade:", values = c("both clades"=both_clades_col, "D clade"=d_clade_col, "G clade"=g_clade_col))+
  theme_minimal() +
  scale_y_continuous(name="number of cases", sec.axis = sec_axis(~ 1/transform*., name = "effective population size")) +
  theme( axis.title.y.right = element_text( angle = 90))+
  coord_cartesian(ylim=c(0.01,1000)) +
  xlab("")
plot(p)
ggsave(plot=p, file=paste(path, 'figures/coal_Ne_vs_test.png', sep='/'), height=5,width=12)



p = ggplot(dat[which(dat$method=="correlated Ne trajectories" & dat$timeframe==timeframename[[1]]),]) + 
...
plot(p)
ggsave(plot=p, file=paste(path, 'figures/bdsky_R0.png', sep='/'), height=5,width=12)



p = ggplot(data=growth[which(growth$method=="correlated Ne trajectories"),]) + 
  geom_vline(xintercept=vline1, alpha=0.5)+
  geom_vline(xintercept=vline2, alpha=0.5)+
  geom_vline(xintercept=vline3, alpha=0.5)+
  geom_text(aes(x=vline1, label="local spread\nannounced", y=text_height), colour="black", angle=90, text=element_text(size=8)) +
...
filename = 'multicoal_skygrowth_1'
...
for (j in seq(2,length(ltt))){
  ...
  # get the clade
  clade_label = clusters[which(clusters$filename==filename & clusters$number==gsub("X","",labels(ltt)[[2]][[j]])),"clade"][[1]]
  
  for (k in seq(1,length(ltt$day))){
    new.dat = data.frame(time = mrsi[which(mrsi$filename==filename), "date"] - offset -ltt[k,"day"],
                         val = ltt[k,j], val_offset = current_max + max_val/2, cluster = as.numeric(gsub("X","",labels(ltt)[[2]][j])), clade=paste(clade_label, "clade"))
...
segment.dat = data.frame(x=as.Date("2020-02-7"), xend=as.Date("2020-02-7"), y=250, yend=350)
segment.dat = rbind(segment.dat, data.frame(x=as.Date("2020-02-7"), xend=as.Date("2020-02-6"), y=250, yend=250))
segment.dat = rbind(segment.dat, data.frame(x=as.Date("2020-02-7"), xend=as.Date("2020-02-6"), y=300, yend=300))
segment.dat = rbind(segment.dat, data.frame(x=as.Date("2020-02-7"), xend=as.Date("2020-02-6"), y=350, yend=350))


p = ggplot(ltt.dat) + geom_ribbon(aes(x=time, ymin=val_offset-val/2, ymax=val_offset+val/2, group=cluster, fill=clade)) +
  ...
  geom_text(aes(x=as.Date("2020-02-4"), label="0", y=250), colour="black", text=element_text(size=8))+
  geom_text(aes(x=as.Date("2020-02-4"), label="50", y=300), colour="black", text=element_text(size=8))+
  geom_text(aes(x=as.Date("2020-02-4"), label="100", y=350), colour="black", text=element_text(size=8))+
  geom_text(aes(x=as.Date("2020-02-8"), label="nr lineages in cluster", y=300), angle=90, colour="black", text=element_text(size=8)) +
  ylab("")

plot(p)
ggsave(plot=p, file=paste(path, 'figures/coal_ltt.png', sep='/'), height=5,width=12)
...
```{r growth_rates}
p_coal_growth = ggplot(growth[which(growth$method=="correlated Ne trajectories" & growth$timeframe=="both clades"),]) + 
...
plot(p_coal_growth)
ggsave(plot=p_coal_growth, file=paste(path, 'figures/growth_rates.png', sep='/'), height=5,width=11)
...
p_R0 = ggplot(growth[which(growth$method=="correlated Ne trajectories"),]) + 
  geom_ribbon(aes(x=time, ymin=R.lower, ymax=R.upper, fill="coalescent skyline"), alpha=0.2) +
  geom_ribbon(aes(x=time, ymin=R.ll, ymax=R.uu, fill="coalescent skyline"), alpha=0.8) +
...
# p_R0_bdsky = 
#   ggplot(dat.bdsky[which(dat.bdsky$timeframe==timeframe[[1]]),]) + 
...
for (i in seq(1,length(mrsi$filename))){
  ...
  if (!startsWith( as.character(mrsi[i,"filename"]),"multibd")){
    ...
    for (j in seq(1,length(time)-average_over,1)){
      ...
      R = 1+growth_rate_val/becoming_uninf
...
p = ggplot()+
  geom_ribbon(data=prev[which(prev$method=="correlated Ne trajectories" & prev$timeframe==timeframename[[1]]),],aes(x=time, ymin=I.lower, ymax=I.upper, fill=timeframe), alpha=0.2)+
  geom_ribbon(data=prev[which(prev$method=="correlated Ne trajectories" & prev$timeframe==timeframename[[1]]),],aes(x=time, ymin=I.ll, ymax=I.uu, fill=timeframe), alpha=0.8)+
  scale_x_date(limits=c(as.Date('2020-02-01'), max(mrsi$date)))  +
...
sims_intros$intro_percentage = as.character(sims_intros$intro_percentage)
...
g.nrintros = ggplot() +
  ...
  theme_minimal()+
  scale_y_log10()+
  ylab("Probability than new sample reveals new introduction") +
  xlab("Number of samples") +
  scale_color_manual(name="percentage of cases\ndue to introductions", 
                     values=c("#F9C988", "#71BD89", "#24A897", both_clades_col))
plot(g.nrintros)
ggsave(plot=g.nrintros, file=paste(path, 'figures/intro_clustersampling.png', sep='/'), height=4,width=7)

g.perc_intros = ggplot(growth[which(growth$method=="correlated Ne trajectories" & growth$timeframe==timeframename[[1]]),]) +
...
```{r prevalence_epiinf}
source(paste(path, 'scripts/plot_Traj.R', sep="/"))
fnames = list(paste(path, 'epiinf_analysis/lc_2.traj', sep="/"),
              paste(path, 'epiinf_analysis/lc_9.traj', sep="/"))
plotTraj(fnames[[1]])
plotTraj(fnames[[2]])
plotMergedTraj(fnames[[1]], fnames[[2]])
```
