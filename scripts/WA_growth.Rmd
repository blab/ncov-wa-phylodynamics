...
p = ggplot(dat[which(dat$method=="correlated Ne trajectories"),]) + 
  ...
  scale_y_continuous(name="number of positive tests", sec.axis = sec_axis(~ 1/transform*., name = "effective population size")) +
...
ggsave(plot=p, file=paste(path, 'figures/bdsky_smoothingcomp.png', sep='/'), height=7,width=12)
...
p_coal_growth = ggplot(growth[which(growth$method=="correlated Ne trajectories"),]) + 
...
plot(p_coal_growth)
ggsave(plot=p_coal_growth, file=paste(path, 'figures/coal_growth.png', sep='/'), height=6,width=15)
...
ggsave(plot=p_R0, file=paste(path, 'figures/R0.png', sep='/'), height=6,width=15)
...
p_R0_bdsky = 
  ggplot(dat.bdsky[which(dat.bdsky$timeframe==timeframe[[2]]),]) + 
...
p = ggplot()+
  # geom_histogram(data=testing, aes(x=date-5,y=count), sta="identity")+
  geom_ribbon(data=prev[which(prev$method=="correlated Ne trajectories" & prev$timeframe==timeframename[[2]]),],aes(x=time, ymin=I.lower, ymax=I.upper, fill=timeframe), alpha=0.2)+
  geom_ribbon(data=prev[which(prev$method=="correlated Ne trajectories" & prev$timeframe==timeframename[[2]]),],aes(x=time, ymin=I.ll, ymax=I.uu, fill=timeframe), alpha=0.8)+
  scale_x_date(limits=c(as.Date('2020-02-01'), max(mrsi$date)))  +
...
nrs = clusters[which(clusters$filename=="multicoal_skygrid_2"),]
...
g.nrintros = ggplot() +
  ...
  scale_color_manual(name="percentage of cases\ndue to introductions", 
                     values=c("#F9C988", "#71BD89", "#24A897", "#3B2668"))
plot(g.nrintros)
ggsave(plot=g.nrintros, file=paste(path, 'figures/sample_intros.png', sep='/'), height=4,width=7)

g.perc_intros = ggplot(growth[which(growth$method=="correlated Ne trajectories"),]) +
...
# g.perc_intros = ggplot(growth[which(growth$method=="correlated Ne trajectories" & growth$timeframe==timeframename[[1]]),]) +
  ...
  xlab("") 
plot(g.perc_intros)
ggsave(plot=g.perc_intros, file=paste(path, 'figures/sample_percentage.png', sep='/'), height=4,width=7)
