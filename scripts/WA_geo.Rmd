...
subset = mobility[which(mobility$region=="North" | mobility$region=="King" | mobility$region=="Pierce" ),]
...
subset = subset[-which(subset$sub_region_2==counties[[a]]),]



p.corr = ggplot(correlation, aes(x=lag, y=1-r))+
  geom_line()+
  facet_wrap(.~county, ncol=3) +
  theme_minimal() +
  xlab("lag time compared to King County in days")+
  ylab("1-correlation coefficient") +
  scale_y_log10()
plot(p.corr)

ggsave(p.corr, file=paste(path, "/figures/mobility_lag.png", sep=""), width=6, height=3)


p.trend = ggplot(subset, aes(x=date, y=val))+
  ...
  # geom_smooth(data=king_county, aes(x=time, y=workplaces_percent_change_from_baseline), color="red")+
  facet_wrap(.~sub_region_2, ncol=3, scales = "free_y") +
  theme_minimal() +
  scale_x_date(limits=c(start,end+14))+
  geom_text(data=subset, aes(x = end, y = -20,label=paste(bestlag,"days")))+
  ylab(mob.val[[10]]) +
  xlab("")
plot(p.trend)
ggsave(p.trend, file=paste(path, "/figures/mobility_trend.png", sep=""), width=8, height=4)
