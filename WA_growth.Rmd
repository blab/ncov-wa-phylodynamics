...
p = ggplot(rep.dat) + geom_histogram(aes(x=time, fill=as.character(cluster)), binwidth=1, position = "stack") +
  scale_x_date(limits=c(as.Date('2020-02-01'), max(mrsi$date)))  +
  ...
  ggtitle('Lineages through time of different local outbreak clusters')
plot(p)
ggsave(plot=p, file=paste(path, 'figures/coal_stacked.pdf', sep='/'), height=5,width=9)
